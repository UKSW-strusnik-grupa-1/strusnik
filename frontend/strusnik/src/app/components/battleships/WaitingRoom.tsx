...
// Auto-start game when all required players are seated
useEffect(() => {
...
return (
    <div className="flex-1 flex flex-col items-center justify-center w-full h-full relative">
        ...
        <div className="absolute inset-0 m-auto w-[40%] h-[30%] flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm rounded-xl border border-white/10 p-4 text-center z-0">
            <h2 className="text-2xl font-bold text-amber-50 mb-2">{t(lang, 'battleships.waiting_room_title')}</h2>
            <p className="text-gray-300 mb-4">
                {t(lang, 'battleships.waiting_for_players')} ({readyPlayersCount}/{maxPlayers})
                <br />
                <span className="text-xs text-gray-500">
                    {maxPlayers === readyPlayersCount ? t(lang, 'battleships.room_full_message') : `Min required: ${maxPlayers === 4 ? 3 : maxPlayers}`}
                </span>
            </p>
            ...
            {isHost ? (
                ...
                <div className="text-green-500/80 text-sm font-mono animate-pulse">
                    {canStart ? t(lang, 'battleships.waiting_for_host') : t(lang, 'battleships.waiting_for_others')}}
                </div>
            )}
        </div>
