      ...
      return (
        ...
        >
          ...
          <p className={`font-bold text-center leading-tight ${isMe ? 'text-amber-400' : 'text-gray-200'}`}>
            {isMe ? t(lang, 'stratego.waiting_room.you') : data.name}
          </p>
          {isMe && (
            <p className="text-[10px] text-green-400 uppercase font-bold mt-1">
              {t(lang, 'stratego.waiting_room.ready')}
            </p>
          )}
        </div>
      );
    ...
    return (
      ...
      >
        ...
        <span className="text-xs uppercase font-bold text-gray-500 group-hover:text-amber-200 mt-1">
          {t(lang, 'stratego.waiting_room.join')}
        </span>
      </button>
    );
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center w-full h-full relative">
      ...
      <div className="absolute inset-0 m-auto w-[40%] h-[30%] flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm rounded-xl border border-white/10 p-4 text-center z-0">
        <h2 className="text-2xl font-bold text-amber-50 mb-2">{t(lang, 'stratego.waiting_room.title')}</h2>

        <p className="text-gray-300 mb-4">
          {t(lang, 'stratego.waiting_room.waiting')} ({readyPlayersCount}/{maxPlayers})
          <br />
          <span className="text-xs text-gray-500">
            {maxPlayers === readyPlayersCount
              ? t(lang, 'stratego.waiting_room.room_full')
              : t(lang, 'stratego.waiting_room.min_required_prefix',) }
          </span>
        </p>
        ...
        {isHost ? (
          ...
          <div className="text-green-500/80 text-sm font-mono animate-pulse">
            {canStart ? t(lang, 'stratego.waiting_room.waiting_host') : t(lang, 'stratego.waiting_room.waiting_players')}
          </div>
        )}
      </div>
