...
export default function Game({ socket, roomId, gameState, myId }: ActiveGameProps) {
  ...
  const my_idx =
    gameState.my_idx !== undefined && gameState.my_idx !== -1 ? gameState.my_idx : calculatedIdx;
  ...
  const clearSetup = () => {
    ...
    setAvailablePieces({ ...SETUP_CONFIG });
    setSelectedPieceToPlace(null);
  };

  const submitSetup = () => {
    if (my_idx === undefined || my_idx === null || my_idx === -1) {
      alert(t(lang, 'stratego.setup.sync_error_refresh'));
      return;
    }
  ...
  const renderGameCell = (r_view: number, c_view: number) => {
    ...
    if (!piece) {
      return (
        <div
          ...
          } ${isLastMoveSrc ? 'bg-yellow-500/10' : ''}`}
    ...
    const pieceColorClass = piece.player === 0 ? 'bg-red-900 border-red-500' : 'bg-blue-900 border-blue-500';
  ...
  if (stage === 'game_over') {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-6 animate-in fade-in duration-700">
        <h2 className="text-5xl text-amber-500 font-bold tracking-widest drop-shadow-lg">
          {t(lang, 'stratego.game_over.title')}
        </h2>

        <div className="text-2xl text-gray-200 bg-black/50 px-8 py-4 rounded-xl border border-amber-900/50">
          {t(lang, 'stratego.game_over.winner_label')}:
          <span className="text-white font-bold text-3xl ml-2">{winner?.name}</span>
          <p className="text-sm text-gray-400 mt-2 text-center italic">{winner?.reason}</p>
        </div>

        <Link href="/lobby/Stratego">
          <button className="px-8 py-3 bg-amber-800 hover:bg-amber-700 text-white font-bold rounded shadow-lg transition-all transform hover:scale-105 cursor-pointer">
            {t(lang, 'stratego.game_over.back_to_lobby')}
          </button>
        </Link>
      </div>
    );
    ...
    if (setup_ready?.[my_idx]) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-amber-100 gap-4">
          ...
          <h2 className="text-2xl font-light tracking-wide">{t(lang, 'stratego.setup.waiting_opponent')}</h2>
        </div>
      );
    }
      ...
      <div className="flex flex-col h-full w-full p-2 overflow-hidden">
                ...
                return (
                  ...
                  >
                    {cell && (
                      ...
                      >
                        <img src={getPieceImgSrc(cell, my_idx)} alt={cell} className="w-full h-full object-fit pointer-events-none" />
        ...
        <div className="flex justify-center gap-4 my-3">
          <button
            ...
            className="px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs uppercase font-bold tracking-wide transition-colors"
          >
          ...
          <button
            onClick={clearSetup}
            className="px-3 py-1 bg-red-900/40 hover:bg-red-900 rounded text-xs uppercase font-bold tracking-wide transition-colors text-red-200"
          >
          ...
          <button
            ...
            className={`px-6 py-1 rounded text-sm font-bold uppercase transition-all shadow-lg
              ${piecesLeft > 0 ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-green-700 hover:bg-green-600 text-white shadow-green-900/50 scale-105'}`}
          >
              ...
              return (
                ...
                >
                  <div className="w-15 h-15 md:w-20 md:h-20 flex items-center justify-center relative pointer-events-none">
                    <img src={getPieceImgSrc(rank, my_idx)} alt={rank} className="w-full h-full object-contain drop-shadow-md" />
                  ...
                  <span className={`text-md mt-1 ${left === 0 ? 'text-red-500' : 'text-gray-400'}`}>
                    {left}/{total}
                  </span>
                </div>
              );
  ...
  if (last_move?.combat) {
    ...
    if (result === 'draw') {
      combatResultText = t(lang, 'stratego.combat.draw');
      combatResultColor = 'text-gray-400';
    } else if (amIAttacker) {
      if (result === 'win') {
        combatResultText = t(lang, 'stratego.combat.win');
        combatResultColor = 'text-green-400';
      } else {
        combatResultText = t(lang, 'stratego.combat.lose');
  ...
  return (
    <div className="relative flex flex-col items-center h-full w-full justify-center">
      ...
      <div className="mb-4 flex flex-col items-center gap-2 z-10">
        <div
          className={`
            ...
            ${isMyTurn ? 'bg-linear-to-r from-green-900 to-green-800 text-green-100 border-green-500 scale-105' : 'bg-gray-900 text-gray-500 border-gray-700'}
          `}
        >
          {isMyTurn ? t(lang, 'stratego.turn.yours') : t(lang, 'stratego.turn.opponent')}
        </div>
      </div>
      ...
      {last_move?.combat && (
        <div className="absolute right-4 top-24 z-50 flex flex-col items-end gap-2 animate-in slide-in-from-right fade-in duration-500">
          <div className="bg-black/80 backdrop-blur-md px-4 py-3 rounded-xl border border-amber-900/50 shadow-2xl flex flex-col items-center gap-2 max-w-[200px]">
            <span className="text-xs text-gray-400 uppercase tracking-widest border-b border-gray-700 w-full text-center pb-1 mb-1">
              {t(lang, 'stratego.combat.title')}
            </span>

            <div className="flex items-center gap-2">
              <div className="flex flex-col items-center">
                <img
                  src={getPieceImgSrc(last_move.combat.attacker.rank, last_move.combat.attacker.player)}
                  className="h-10 w-10 object-contain drop-shadow-[0_0_5px_rgba(255,0,0,0.5)]"
                  alt={last_move.combat.attacker.rank}
                />
                <span className="text-[10px] text-gray-500 mt-1">{t(lang, 'stratego.combat.attack')}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <img
                  src={getPieceImgSrc(last_move.combat.defender.rank, last_move.combat.defender.player)}
                  className="h-10 w-10 object-contain drop-shadow-[0_0_5px_rgba(0,0,255,0.5)]"
                  alt={last_move.combat.defender.rank}
                />
                <span className="text-[10px] text-gray-500 mt-1">{t(lang, 'stratego.combat.defense')}</span>
              </div>
            </div>

            <div className={`text-sm font-bold uppercase tracking-wide mt-1 ${combatResultColor}`}>{combatResultText}</div>
          </div>
        </div>
      )}
      ...
      <div className="mt-6 flex gap-12 text-xs font-mono tracking-widest text-gray-500 opacity-80">
        <div className="flex items-center gap-2">
          <div className={`w-3 h-3 rounded-full ${my_idx === 0 ? 'bg-red-600 shadow-[0_0_10px_red]' : 'bg-blue-600 shadow-[0_0_10px_blue]'}`} />
          {t(lang, 'stratego.footer.you')} ({seats?.[my_idx]?.name})
        </div>

        <div className="flex items-center gap-2">
          <div className={`w-3 h-3 rounded-full ${my_idx === 0 ? 'bg-blue-600' : 'bg-red-600'}`} />
          {t(lang, 'stratego.footer.enemy')} ({seats?.[1 - my_idx]?.name})
        </div>
      </div>
    </div>
  );
